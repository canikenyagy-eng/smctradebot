...
LOG_FILE="${STRUCTURE_QUALITY_CALIBRATION_LOG:-logs/structure_quality_calibration.out.log}"
if [[ "${STRUCTURE_QUALITY_CALIBRATION_SELF_LOG:-1}" == "1" ]]; then
  : > "$LOG_FILE"
  exec > "$LOG_FILE" 2>&1
fi

PID_FILE="logs/structure_quality_calibration.pid"
if [[ -f "$PID_FILE" ]]; then
  existing_pid="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$existing_pid" ]] && kill -0 "$existing_pid" 2>/dev/null; then
    echo "structure quality calibration job is already running with PID ${existing_pid}"
    exit 0
  fi
fi
...
OUTPUT="${STRUCTURE_QUALITY_CALIBRATION_OUTPUT:-reports/structure_quality_calibration_1200_3000_step2.json}"
...
echo "Started structure quality calibration at ${started_at}"
echo "Root: ${ROOT_DIR}"
echo "Log: ${LOG_FILE}"
echo "Output: ${OUTPUT}"
echo "Pairs: ${PAIRS}"
...
echo
echo "Finished structure quality calibration at ${finished_at}"
